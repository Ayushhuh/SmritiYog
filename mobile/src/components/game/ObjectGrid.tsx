...
export function ObjectGrid({ choices, status, chosenIndex, onChoose }: ObjectGridProps) {
  ...
  return (
    <View
      ...
      {choices.map((choice, index) => {
        const centerSingleInLastRow = columns === 2 && choices.length === 5 && index === 4;
        return (
          <View
            key={choice.index}
            style={centerSingleInLastRow ? styles.centeredTile : styles.tile}>
            <ObjectChoice
              index={choice.index}
              label={choice.label}
              icon={choice.icon}
