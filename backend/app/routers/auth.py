...
from app.dependencies import get_current_user, require_caregiver
...
def _auth_response(user: models.User) -> schemas.AuthResponse:
    ...
    return schemas.AuthResponse(access_token=token, role=user.role, user=user)
...
@router.get("/me/caregiver", response_model=schemas.UserRead)
async def me_caregiver(user: models.User = Depends(require_caregiver)) -> models.User:
    return user